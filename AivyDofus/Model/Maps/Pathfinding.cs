...
namespace AivyDofus.Model.Maps
{
    /// <summary>
    /// 
    /// </summary>
    ...
    public static class Pathfinding
    {
        ...
        /// <returns></returns>
        public static int GetPathLength(Map map, int beginCell, string encodedPath)
        {
        ...
        /// <param name="Cell"></param>
        /// <returns></returns>
        public static Point GetPoint(Map map, int cell)
        {
        ...
        /// <param name="map"></param>
        /// <returns></returns>
        public static int[] GetDirectionChanges(Map map)
        {
            if (MapDirections.ContainsKey(map.MapId))
                return MapDirections[map.MapId];
            ...
            MapDirections.Add(map.MapId, directions);
        ...
        /// <returns></returns>
        public static int GetDirection(Map map, int beginCell, int dndCell)
        {
        ...
        /// <param name="direction"></param>
        /// <returns></returns>
        public static int NextCell(Map map, int cellId, int direction, int length = 1)
        {
            switch (direction)
            {
                case 0:
        ...
        /// <param name="encodedPath"></param>
        /// <returns></returns>
        //public static MovementPath IsValidPath(AbstractEntity entity, Map map, int currentCell, string encodedPath)
        //{
        ...
        ///// <param name="encodedPath"></param>
        ///// <returns></returns>
        //public static MovementPath IsValidPath(AbstractFight fight, Fighter fighter, int currentCell, string encodedPath)
        //{
        //    if (encodedPath == "")
        //        return null;
        ...
        /// <param name="endCell"></param>
        /// <returns></returns>
        //public static int IsValidLine(AbstractEntity entity, Map map, MovementPath finalPath, int beginCell, int direction, int endCell, int finalCell)
        //{
        //    var length = -1;
        ...
        ///// <param name="endCell"></param>
        ///// <returns></returns>
        //public static int IsValidLine(AbstractFight fight, Fighter fighter, MovementPath path, int beginCell, int direction, int endCell)
        //{
        //    var length = -1;
        //    var actualCell = beginCell;
        ...
        ///// <param name="cellId"></param>
        ///// <returns></returns>
        //public static IEnumerable<Fighter> GetEnnemiesNear(AbstractFight fight, FightTeam team, int cellId)
        //{
        ...
        /////// <param name="cellId"></param>
        /////// <returns></returns>
        //public static List<Fighter> GetFightersNear(Fight fight, int cellId)
        //{
        //    List<Fighter> fighters = new List<Fighter>();
        //    foreach (var direction in Pathfinding.FIGHT_DIRECTIONS)
        //    {
        //        var fighter = fight.GetFighterOnCell(Pathfinding.NextCell(fight.M, cellId, direction));
        //        if (fighter != null)
        ...
        public static bool BresenhamLine(Fight fight, int beginCell, int endCell)
        {
        ...
        private static bool BresenhamLine(Fight fight, int beginCell, int endCell, int x0, int y0, int x1, int y1)
        {
            bool steep = Math.Abs(y1 - y0) > Math.Abs(x1 - x0);
            if (steep)
            {
            ...
            for (int x = x0; x <= x1; x++)
            {
                ...
                {
                    var fightCell = fight.Map.Cells[cellId];
                    if (fightCell == null)
                        return false;
                    //logger.Info($"[Path] : {cellId} {fightCell.IsInLineOfSight}");
                    if (fightCell.IsInLineOfSight)
                        return false;
                    if (fight.Fighters.Values.Where(c => c.IsAlive).Select(c => c.Cell).Contains(fightCell))
                        return false;
                }
                //logger.Info($"[Path 2] : {cellId}");
                error += deltay;
                if (2 * error >= deltax)
                {
                    y += ystep;
        ...
        /// <param name="y"></param>
        /// <returns></returns>
        public static int GetCell(Map map, double x, double y)
        {
        ...
        private IEnumerable<int> FindPath(int StartCell, int EndCell, bool Diagonal, int MovementPoints = -1, IEnumerable<int> Obstacles = null)
        {
            ...
            while (OpenList.Count > 0 && !Success)
            {
                ...
                for (int i = 0; i <= (Diagonal ? 8 - 1 : 4 - 1); i++)
                {
                    ...
                    if ((!map.Cells[NewLocation].IsWalkable || (Obstacles != null && Obstacles.Contains(NewLocation))) && NewLocation != EndCell)
                    {
                        continue;
                    }
